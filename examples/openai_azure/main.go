...
// BasicUsageExample demonstrates creating an Azure OpenAI client and making a simple prompt call.
...
func ErrorHandlingExample(factory *client.ClientFactory) {
	...
	// Use missing/invalid config to trigger an error
	config := &types.AIConfig{
		Provider:    "openai-azure",
		BaseURL:     os.Getenv("OPENAI_AZURE_ENDPOINT"),
		Model:       os.Getenv("OPENAI_AZURE_MODEL"),
...
func main() {
	fmt.Println("Azure OpenAI - Basic Usage Examples")
	fmt.Println("====================================")
	...
	// (e.g., go run examples/openai_azure_client/main.go).
	...
	fmt.Println("\nAll Azure OpenAI examples completed")
}
